...
class CalculateRatingFixoInitialEpoca:
    ...
    def __read_file(self):
        self.rating_macro = pd.read_csv(
            self.path + cons_path.path_rating_macro + cons_path.corrected_rating_macro + cons_path.csv, sep=",",
            encoding='unicode_escape')

    def __write_file(self):
        self.output.to_csv(
            self.path + cons_path.path_rating_macro + cons_path.rating_fixo_macro + cons_path.csv, \
            index=False)
