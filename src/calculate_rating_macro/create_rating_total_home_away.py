...
class CreateRating:
    ...
    def __read_input(self, year: int) -> pd.DataFrame:
        ...
        :return:
        """
        df = pd.read_csv(self.path + cons_path.path_odds_stats + cons_path.stats + str(year) + cons_path.csv, sep=",",
    ...
    def __read_teams(self):
        ...
        :return:
        """
        df = pd.read_csv(self.path + cons_path.path_teams + cons_path.teams + cons_path.csv, sep=",",
                         encoding='unicode_escape', index_col=0)
        return df
    ...
    def calculate_rating(self, team: str, year: int) -> list:
        """
    ...
    def __write_output(self) -> None:
        with open(self.path + cons_path.path_rating_macro + cons_path.rating_macro + cons_path.csv, "w",
                  newline="") as f:
            writer = csv.writer(f)
